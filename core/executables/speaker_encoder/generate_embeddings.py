...
from utils.oputils import preprocess_wav, wav_to_mel_spectrogram   # We want to expose this function from here
...
if __name__ == '__main__':
    ...
    parser.add_argument("-e", "--enc_model_fpath", type=Path,
                        default=f"{hp.models_save_path}{hp.f_delim}spk_encoder{hp.f_delim}pretrained.pt",
                        help="Path to a saved encoder")
    parser.add_argument("-s", "--syn_model_dir", type=Path,
                        default=f"synthesizer{hp.f_delim}saved_models{hp.f_delim}logs-pretrained{hp.f_delim}",
                        help="Directory containing the synthesizer model")
    parser.add_argument("-v", "--voc_model_fpath", type=Path,
                        default=f"vocoder{hp.f_delim}saved_models{hp.f_delim}pretrained{hp.f_delim}pretrained.pt",
    ...
    parser.add_argument("--no_sound", action="store_true", default=True,
                        help="If True, audio won't be played.")
    parser.add_argument("-a", "--audio_fpath", type=Path,
                        default=f"{hp.wav_path}{hp.f_delim}",
                        help="Path to wave files")
    parser.add_argument("-m", "--embed_fpath", type=Path,
                        default=f"{hp.wav_path}{hp.f_delim}vctk_mol{hp.f_delim}spk_embeds{hp.f_delim}",
    ...
    if not os.path.exists(args.embed_fpath):
        os.mkdir(args.embed_fpath)
    ...
    try:
        ...
        wav_files = get_files(args.audio_fpath, ".wav")
        ...
        # only use this function (with its default parameters):
            embed = embed_utterance(
                preprocessed_wav)
            embed_path = args.embed_fpath / \
                filename.split(hp.f_delim)[-1].replace('.wav', '.npy')
